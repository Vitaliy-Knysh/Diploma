...
Препятствия обнаруживает вращающийся лазерный дальномер. Он делает около 3 оборотов секунду и измеряет расстояние 360 раз за оборот. 
Дальномер действует в полярных координатах, измерения в миллиметрах. Чтобы отрисовать проекцию обнаруженных препятствий на плоскость, необходимо перевести миллиметры в пиксели(так как координаты задает камера), и перейти в Декартовы координаты. Так выглядит картинка с дальномера, перед которым в произвольком порядке выставлены небольшие коробочки:
![image](https://user-images.githubusercontent.com/75195148/188921960-7225f17e-9ee6-4200-afc1-1d99ad7800a5.png)

Красный круг - дальномер, синие точки - контуры обнаруженных препятствий.

Если наложить картинку с дальномера на картинку с камеры, получается так:
![image](https://user-images.githubusercontent.com/75195148/188923176-57d686a5-344f-442a-8346-336520de6e2a.png)

## Почему точки сдвинуты и как с этим бороться? ##
Сдвиг точек появился потому, что объектив камеры дает искажение типа "рыбий глаз". OpenCV позволяет убрать искажение, но не до конца. Получается, что "врёт" здесь не дальномер, а камера. Проблема в том, что только через камеру можно задать координаты.

Чтобы привести показания дальномера ближе к искажённой картинке с камеры, были введены три поправочных коэфициента:
1. Относительно положения робота. К краям картинка "растягивается", коэфициент это компенсирует.
2. Относительно угла ориентации робота. Дальномер находится не в центре робота, а слегка впереди. Чтобы показать расстояние от робота до препятствия, "центр" измерения расстояния необходимо сместить.
3. Эмпирически полученный коэфициент смещения "центра" измерения.

Такая картина получается без поправочных коэфициентов:
![image](https://user-images.githubusercontent.com/75195148/188926239-16e3af48-c5b8-4223-a933-ef5d1db48ded.png)

А такая - с коэфициентами:
![image](https://user-images.githubusercontent.com/75195148/188926343-34f5c2c4-033e-4244-ba1e-89079246ee21.png)

Черные линии - это центр исходного изображения, нужно было для отладки.

# Итоги работы #
1. Разработан алгоритм движения робота без собственной одометрии.
2. Реализовано клиент-серверное приложение для управления роботом.
3. Реализовано ПО для обработки данных с вращаюшегося лазерного дальномера.
4. Реализован волновой алгоритм для движения в среде с препятствиями.
5. Робот доехал до груза, забрал его и приехал с ним в точку назначения, избегая препятствия.
