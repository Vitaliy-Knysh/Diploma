...
endPoint = (6, 1)
tileSize = 130  # расстояние между вершинами по горизонтали и вертикали  в пикселях
...
# я мог бы объединить эти три списка, но так проще разобраться в них.
pathFin = []  # список, содержащий путь через граф
# или связь, заблокированную препятствием
obstacleLine = [[[300, 300], [100, 55]], [[750, 600], [700, 150]]]
...
for x in range(tilesX):  # присвоение координат вершинам графа
    for y in range(tilesY):
        graphCoord[y][x] = (100 + (tileSize * x), 100 + (tileSize * y))
print(graphCoord)
...
cv2.line(map, obstacleLine[0][0], obstacleLine[0][1], (0, 0, 255), 2)
cv2.line(map, obstacleLine[1][0], obstacleLine[1][1], (0, 0, 255), 2)
...
i = 6
while i > -1:
    print(graphCoord[pathFin[i][1]][pathFin[i][0]])
    i -= 1
